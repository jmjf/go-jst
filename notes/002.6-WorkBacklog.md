...
In `common/errors.go`, `LogError()` needs to deal with a case where we get an array of errors from `PropsError`. It converts the error data to a JSON string so it can decide if the error data will translate right. I was always converting that to a string, so the logger got a string. Now, it uses the original error data unless JSON conversion gives an empty JSON object. Also simplified the "empty JSON" conversion case to just `Sprintf` it (vs. previously stripping of `[]`). I can live with the `[]` for simpler code.

Possible future issues:

* How will it deal with `map[string]any` that the repo `get` functions return?
* How will it deal with nested structures that do convert to JSON correctly?

Let's test it in the playground. (Result: Looks like the output makes sense.)
...
type User struct {
 ...
 TestMap     map[string]any `json:"test_map"`
}
...
**COMMIT:** FIX: format errorData object in log output as an object, not a string, so it's friendlier to structured log indexers

**COMMIT:** CHORE: run go mod tidy everywhere and go work sync to ensure everything is up to date

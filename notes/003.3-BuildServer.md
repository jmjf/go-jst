# Server -- Reorganize server setup and startup
...
## Back to the route handler

The `Init()` methods return a controller, among other things. I can use that to set up the route handler.

In `jobStatus/infra/httpHandler.go`, write `Handler(logger *slog.Logger, ctrl jobStatus.Controller) http.Handler`. It creates a sublogger for the request and `switch`es to decide which controller to call. In the future, `Init()` will need to return more than one controller. I may try to return a `map[string]*jobStatus.Controller` and use the key to get the right controller in the `case`.

In `/cmd/httpServer-dbpg/main.go` build the muxes.

After some failures, I found I can't use `apiMux`/`subMux` to do partial path matching, so I removed `apiMux` and put the prefix on the `subMux` paths. It works. I've added a `loggerHandler` in `main.go` that logs the route, attached to `"/"` route (so anything not matched). I'll remove that when I build request logging middleware.

**COMMIT:** REFACTOR: move HTTP handler to jobStatus/infra; use a ServeMux

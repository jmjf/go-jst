...
I have an issue because the logger on the controller doesn't have the route specific data. I need to pass the logger to the controller method. Which is setting me up nicely to move JSON encode/decode, etc., into `httpHandler`. I'm not sure where that will lead `Controllers`, if it will stay to hold the UC or if I'll let the handler know about the UC and make the controller generic.

**COMMIT:** REFACTOR: pass logger to controller methods so they get the route-specific logger

Now, what would be involved in moving all the `res` and `req` uses out of the controller into the handler?

* Move code from the controller `Add()` and `GetByQuery` methods to the handler.
  * Probably keep them separate functions in the handler, but local to the handler package.
* Change the handler to accept a UC instead of a controller.

Do I move `callUseCase()` into the use case? I think not. Logging is an adapter-layer concern because many logging, metrics, telemetry and other observability tools and frameworks exist. If they move into the business layers, it puts infrastructure and details of the observability mechanics in the business layers. The business layers should return errors with enough detail to support the adapter layer owning logging and metrics.

I'm going to take the debug logs out of `callUseCase()`, which removes the "get caller name" code. I still need the logger to pass to `internal.LogError()`, but if I define an unknown error, I could return it and move logging out of `callUseCase()` entirely. (Decided to do this and have.)

Do I move handler creation into `modinit` and make it the controller? No, because that would mean `modinit` needs to know about the different communication protocols the infrastructure supports. For example, if the application supports both HTTP (JSON) and gRPC (protobufs), `modinit` would need to know about both and provide controllers for both. The changes here are separating the infrastructure and protocol gnostic code (decode/encode JSON, etc.) from the protocol agnostic code (calling use cases and setting up errors).

So, in summary:

* Controllers are split between infrastructure specific and infrastructure neutral code.
* Infrastructure specific code is responsible for decoding/encoding data for a specific protocol and logging errors.
  * Currently, the only case is in `jobStatus/http/httpHandler.go`, but I can see a gRPC version in the future when I try that.
* Infrastructure neutral code is responsible for calling the use case and deciding a general return type (OK, bad request, server error)

I suspect this split doesn't make as much sense as it seemed to when I started it. I suspect it's better to let the protocol specific code decide how to classify an error, which will make the infrastructure neutral code too small to be worth separating. But it's late, so I'll look at it tomorrow.

**COMMIT:** REFACTOR: move protocol gnostic controller logic into protocol specific code (httpHandler)

Looking at `CallUseCase()` after a few hours away, all it does is set the HTTP response status, which belongs in the HTTP layer.

* Move list of "bad request" errors from `internal/errors.go` to `internal/jobStatus/http/httpHandler.go`.
  * If later work shows this list can be common across modules, I'll move it back up and name it `HTTPBadRequestErrors` or similar.
* Move the code that chooses the HTTP response status to `httpHandler.go` as `errToHTTPStatus()` and use it to get the response status.
* Add a `logError()` function to handle error logging details. Replace duplicate error logging code with it.
* Simplify handler and controller code in `httpHandler.go`.
* Delete `controllers.go` because it's no longer needed.

I still have a controller concept, but controllers are now functions in `httpHandler.go` and are unexported. The handler is a thin shim that uses request HTTP methods and request details to decide which controller function to run. Controller functions are responsible for decoding request bodies, calling the use case,
setting HTTP response status, Jencoding response bodies, handling any errors, etc.

It's working except the errors aren't showing the full call trace. It's late, so I'll look at that tomorrow.

**COMMIT:** REFACTOR: simplify handler and controller function code; controller functions now entirely in httpHandler.go
